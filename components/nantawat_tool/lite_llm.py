...
class ChatLiteLLMModelComponent(LCModelComponent):
    ...
    inputs = [
        ...
        DropdownInput(
            ...
            options=["OpenAI", "Azure", "Anthropic", "Replicate", "Cohere", "OpenRouter", "bedrock", "None"],
            value="None",
        ),
        FloatInput(
            name="temperature",
    ...
    def build_model(self) -> LanguageModel:  # type: ignore[type-var]
        ...
        model = f"{self.provider.lower()}/{self.model}" if provider else self.model
        output = ChatLiteLLM(
            model=model,
            client=None,
        ...
        result = output.invoke("Say hello!")
        print(result)
        return output
