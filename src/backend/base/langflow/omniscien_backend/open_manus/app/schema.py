...
class Memory(BaseModel):
    ...
    def add_message(self, message: Message) -> None:
        ...
        if len(self.messages) > self.max_messages:
            # If the oldest messages are an assistant and tool message, remove both
            if self.messages[0].role == "assistant" and self.messages[1].role == "tool":
                self.messages = self.messages[2:]  # Remove oldest assistant and tool messages

            else:  # Remove only the oldest message
