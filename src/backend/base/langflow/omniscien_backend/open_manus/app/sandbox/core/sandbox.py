...
class DockerSandbox:
    ...
    async def run_command(self, cmd: str, timeout: int | None = None) -> str:
        ...
        try:
            ...
            raise SandboxTimeoutError(f"Command execution timed out after {timeout or self.config.timeout} seconds")
    ...
    async def write_file(self, path: str, content: str) -> None:
        ...
        try:
            ...
            # Prepare file data
            tar_stream = await self._create_tar_stream(os.path.basename(path), content.encode("utf-8"))
